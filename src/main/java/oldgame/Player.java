...
public class Player extends Entity {
    ...
    public void getPlayerImage() {
        // Load images from assets folder (marked as resource folder in IntelliJ
        try {
    ...
    public void update() {
        if (keyH.upPressed || keyH.downPressed || keyH.leftPressed || keyH.rightPressed) {
            // Input Handler
            // Coordinates for y-axis is flipped; increases downward
            if (keyH.upPressed) {
                direction = "up";
                worldY -= speed;
            }
            ...
            // Walk cycle logic
            spriteCounter++;
            if (spriteCounter > 12) {  // Change frequency with num on right
                if (spriteNum == 1) {
                    spriteNum = 2;
                }
                else if (spriteNum == 2) {
...
//        g2.fillRect(x, y, gp.tileSize, gp.tileSize);
        ...
        // drawImage method does what you think it does
